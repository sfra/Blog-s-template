<?php $configJSON = json_decode(file_get_contents("../config/mexico.json"),true);

$language = json_decode(file_get_contents('config/mexico.json'),true)['language'];
$translations = json_decode(file_get_contents("translations/$language.json"),true);
?>

<header id="header" class="alt">
    <a id="main-title" href="index.php" class="logo"><strong>
    <?=$translations['reset-password'][0]?></strong> <span><?=$translations['reset-password'][1]?></span>
            </a>

    <nav class="back" style="z-index:700;">
        <div >
            <a href="index.php"><?=$translations['back']?></a>
        </div>
    </nav>
        <img alt="logo" id="eagle" src="data:image/png;base64,
QmCC
" />
</header>