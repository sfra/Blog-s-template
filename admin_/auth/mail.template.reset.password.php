<html>
                <head>
                <meta charset="utf8 />
                <style>
                body {
                    background: rgb(255,255,255);
                }
                    h1 {
                        position: relative;
                        z-index: 100;
                        top: -140px;
                        left: 300px;
                        background: rgba(200,200,200,1);
                        color: #000000;
                        padding: 20px;
                    }
                    h2 {
                        position: relative;
                        z-index: 100;
                        top:-80px;
                        background: rgb(200,200,200);
                        padding: 20px;
                    }
                </style>
                </head>
                <body>

                <img src="data:image/png;base64,
QmCC
" alt="logo" />
                    <h1>Witaj {login}</h1>
                    <h2>Poniżej znajduje się link resetujący hasło do serwisu aktywnastronameksyku.pl</h2>
                    <a href="{link}">Resetuj hasło</a>
                    Jeśli uważasz, że to nieporozumienie, <a href="{unlink}">kliknij</a>
                    
                </body>
</html>               